...
def inscrire_utilisateur(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            utilisateur.set_password(form.cleaned_data['password'])  # hash le mot de passe
            utilisateur.save()

            # Affecter le rôle
            role = form.cleaned_data['role']
            if role == 'technicien':
                Technicien.objects.create(utilisateur=utilisateur, groupe_affectation='N/A', responsabilites='À définir')
          
            elif role == 'administrateur':
                Administrateur.objects.create(utilisateur=utilisateur, administration='Administration générale')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # USER : pas besoin de table spécifique pour l'instant
...
from django.shortcuts import render, redirect
from django.contrib import messages
from .forms import TypeEquipementForm, EquipementBaseForm, EquipementIndividuelForm, EquipementDepartementalForm, EquipementReseauForm
from django.shortcuts import render, redirect
from django.contrib import messages
...
from django.shortcuts import render, redirect
from django.contrib import messages
from .forms import TypeEquipementForm, EquipementBaseForm, EquipementIndividuelForm, \
                   EquipementDepartementalForm, EquipementReseauForm
...
def ajouter_equipement(request):
    ...
    # Déterminer le type d'équipement sélectionné
    type_equipement = request.POST.get('type_equipement') if request.method == 'POST' else None
    
    # Initialisation conditionnelle des formulaires spécifiques
    spec_forms = {
        'INDIVIDUEL': EquipementIndividuelForm(request.POST if type_equipement == 'INDIVIDUEL' else None),
        'DEPARTEMENTAL': EquipementDepartementalForm(request.POST if type_equipement == 'DEPARTEMENTAL' else None),
        'RESEAU': EquipementReseauForm(request.POST if type_equipement == 'RESEAU' else None),
    }

    if request.method == 'POST':
        if type_form.is_valid() and base_form.is_valid():
            type_equipement = type_form.cleaned_data['type_equipement']
            form_specifique = spec_forms[type_equipement]
            
            if form_specifique.is_valid():
                try:
                    # Sauvegarde de l'équipement de base
                    equipement = base_form.save(commit=False)
                    equipement.type_equipement = type_equipement
                    equipement.save()  # Sauvegarde initiale pour obtenir un ID
                    
                    # Sauvegarde de l'équipement spécifique
                    sous_equipement = form_specifique.save(commit=False)
                    sous_equipement.equipement = equipement  # Liaison OneToOne
                    sous_equipement.save()  # Sauvegarde finale
                    ...
                    # Rollback implicite car dans une transaction
            else:
                messages.error(request, "Erreur dans le formulaire spécifique. Veuillez vérifier les données.")
                # Debug: Afficher les erreurs dans la console
                print("Erreurs formulaire spécifique:", form_specifique.errors)
        else:
    ...
    return render(request, 'utilisateur/ajoutequip.html', {
        'type_form': type_form,
        'base_form': base_form,
        'spec_forms': spec_forms,
...
def liste_equipements(request):
    ...
    return render(request, 'utilisateur/liste_equipements.html', {
